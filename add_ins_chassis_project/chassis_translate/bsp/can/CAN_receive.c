...
/* 静态全局变量，用于存储从云台板通过CAN2传来的Yaw轴电机数据 */
static motor_measure_t yaw_motor_from_gimbal;
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
    ...
    /*========== 新增：接收欧拉角 ==========*/
    if (hcan->Instance == CAN2 && rx_header.StdId == 0x303) {
    ...
    /*========== 新增：接收从云台发来的 Yaw 电机数据 ==========*/
    if (hcan->Instance == CAN2 && rx_header.StdId == 0x304)
    {
        yaw_motor_from_gimbal.last_ecd = yaw_motor_from_gimbal.ecd;
        yaw_motor_from_gimbal.ecd = (uint16_t)((rx_data[0] << 8) | rx_data[1]);
        yaw_motor_from_gimbal.speed_rpm = (int16_t)((rx_data[2] << 8) | rx_data[3]);
        // 如果需要 last_ecd 也可以在这里解析
        return;
    }
...
void CAN_send_camera_signal(int16_t ch3, int16_t ch4, int16_t ch6, int16_t ch8) {
    CAN_TxHeaderTypeDef tx_header;
    uint8_t send_data[8];
    uint32_t send_mailbox;

    // ����CAN��Ϣͷ
    tx_header.StdId = 0x301; // �Զ���CAN ID
...
//can2发送陀螺仪欧拉角数据
// void CAN_send_bim088_signal(int16_t ch3, int16_t ch4, int16_t ch6, int16_t ch8)
// {
//     CAN_TxHeaderTypeDef tx_header;
//     uint8_t send_data[8];
//     uint32_t send_mailbox;
...
/**
  * @brief  获取从云台板传来的Yaw轴电机数据指针
  * @param  无
  * @retval 指向 motor_measure_t 结构体的指针
  * @note   底盘控制器通过此接口获取云台当前的编码器位置和转速
  */
const motor_measure_t *get_yaw_motor_measure_from_gimbal_point(void)
{
    return &yaw_motor_from_gimbal;
}